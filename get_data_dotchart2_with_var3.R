get_data_var = function(n_years_to_av,BMafterRemoval_H,BMafterRemoval_C,BMafterRemoval_O,phases_timelines,calc_sd=FALSE){
#get_data = function(wd,n_years_to_av,BMafterRemoval_H,BMafterRemoval_C,BMafterRemoval_O,phases_timelines){
  ...
  for(i in 1:length(phases_timelines)){
    ...
    herb_se_last_months_kg = c(herb_se_last_months_kg,se(tl$TotalBiomass_kg)/mean(tl$TotalBiomass_kg)) # insert se of monthly biomasses
  }
  
  # endo carnivores
  carn_mean_last_months_kg = c()
  carn_se_last_months_kg = c()
  for(i in 1:length(phases_timelines)){
    tl = rbind.fill(get(phases_timelines[i]))
    months_to_keep = (max(tl$Month)-n_years_to_av*12+1):max(tl$Month)
    tl = tl[tl$Month%in%months_to_keep,] # keep last n_years_to_av only
    ...
    carn_se_last_months_kg = c(carn_se_last_months_kg,se(tl$TotalBiomass_kg)/mean(tl$TotalBiomass_kg)) # insert se of monthly biomasses
  }
  
  # endo omnivores
  omni_mean_last_months_kg = c()
  omni_se_last_months_kg = c()
  for(i in 1:length(phases_timelines)){
    tl = rbind.fill(get(phases_timelines[i]))
    months_to_keep = (max(tl$Month)-n_years_to_av*12+1):max(tl$Month)
    tl = tl[tl$Month%in%months_to_keep,] # keep last n_years_to_av only
    ...
    omni_se_last_months_kg = c(omni_se_last_months_kg,se(tl$TotalBiomass_kg)/mean(tl$TotalBiomass_kg)) # insert se of monthly biomasses
  }
  ...
  all_heterotrophs_se_last_months_kg = c()
  for(i in 1:length(phases_timelines)){
    all_heterotrophs_se_last_months_kg = c(all_heterotrophs_se_last_months_kg,mean(c(omni_se_last_months_kg[i],carn_se_last_months_kg[i],herb_se_last_months_kg[i])))
  }
 
  # mega endo herbivores only
  herb_mega_mean_last_months_kg = c()
  herb_mega_se_last_months_kg = c()
  for(i in 1:length(phases_timelines)){
    tl = rbind.fill(get(phases_timelines[i]))
    months_to_keep = (max(tl$Month)-n_years_to_av*12+1):max(tl$Month)
    tl = tl[tl$Month%in%months_to_keep,] # keep last n_years_to_av only
    ...
    herb_mega_se_last_months_kg = c(herb_mega_se_last_months_kg,se(tl$TotalBiomass_kg)/mean(tl$TotalBiomass_kg)) # insert se of monthly biomasses
  }
  
  # mega endo carnivores only
  carn_mega_mean_last_months_kg = c()
  carn_mega_se_last_months_kg = c()
  for(i in 1:length(phases_timelines)){
    tl = rbind.fill(get(phases_timelines[i]))
    months_to_keep = (max(tl$Month)-n_years_to_av*12+1):max(tl$Month)
    tl = tl[tl$Month%in%months_to_keep,] # keep last n_years_to_av only
    ...
    carn_mega_se_last_months_kg = c(carn_mega_se_last_months_kg,se(tl$TotalBiomass_kg)/mean(tl$TotalBiomass_kg)) # insert se of monthly biomasses
  }
  
  # mega endo omnivores only
  omni_mega_mean_last_months_kg = c()
  omni_mega_se_last_months_kg = c()
  for(i in 1:length(phases_timelines)){
    tl = rbind.fill(get(phases_timelines[i]))
    months_to_keep = (max(tl$Month)-n_years_to_av*12+1):max(tl$Month)
    tl = tl[tl$Month%in%months_to_keep,] # keep last n_years_to_av only
    ...
    omni_mega_se_last_months_kg = c(omni_mega_se_last_months_kg,se(tl$TotalBiomass_kg)/mean(tl$TotalBiomass_kg)) # insert se of monthly biomasses
  }
  ...
  t1_se = se(t1$TotalStockBiomass)/mean(t1$TotalStockBiomass)
  t2_se = se(t2$TotalStockBiomass)/mean(t2$TotalStockBiomass)
  t3_se = se(t3$TotalStockBiomass)/mean(t3$TotalStockBiomass)
  t1 = mean(t1$TotalStockBiomass)
  t2 = mean(t2$TotalStockBiomass)
  t3 = mean(t3$TotalStockBiomass)
  veg = c(t1,t2,t3)
  veg_se = c(t1_se,t2_se,t3_se)
 ...
 var = c(veg_se,
             all_heterotrophs_se_last_months_kg,
             herb_se_last_months_kg,
             carn_se_last_months_kg,
             omni_se_last_months_kg,
             herb_mega_se_last_months_kg,
             carn_mega_se_last_months_kg,
             omni_mega_se_last_months_kg)
